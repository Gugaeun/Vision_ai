...
| [`Requirements_DINO/`](Requirements_DINO) · [`Requirements_YOLO/`](Requirements_YOLO) | 냉장 진열대 이상(상한 과일, 곰팡이, 포장 훼손 등) 탐지 데모. Grounding DINO와 YOLO-World 두 가지 Open-Vocabulary 탐지 모델로 각각 구현해 비교 |
...
같은 UI/UX로, **서로 다른 두 가지 Open-Vocabulary(개방형 어휘) 탐지 모델**을 각각 구현해 비교해볼 수 있도록 만들었습니다.
...
```bash
# Grounding DINO 버전
cd Requirements_DINO
pip install -r requirements.txt
python app.py

# 또는 YOLO-World 버전
cd Requirements_YOLO
pip install -r requirements.txt
python app.py
```
...
![테스트에 쓴 정상/곰팡이 토마토 사진](docs/images/test-tomatoes.png)

정상 토마토와 곰팡이 핀 토마토 사진 한 장([`docs/images/test-tomatoes.png`](docs/images/test-tomatoes.png))에, 프롬프트 6개(`rotten fruit`, `moldy vegetable`, `damaged packaging`, `dented can`, `torn label`, `spilled liquid`)를 그대로 넣고 임계값을 0.01까지 낮춰서 **각 모델이 두 토마토에 실제로 어떤 신뢰도를 매기는지** 확인했습니다.

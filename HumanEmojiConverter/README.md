...
`06_PoseEstimation`의 YOLO-Pose + XGBoost 파이프라인(13~16번 스크립트)을 그대로 가져와서, 자세를 3가지 "기분"으로 분류하고 실시간으로 이모지 얼굴을 띄워주는 개인 프로젝트입니다.

**왜 필요한가**: 화상수업·스트리밍 중에 마이크를 켜지 않고도 지금 내 상태(생각 중/신남/지침)를 자세만으로 표현할 수 있으면 재미있고 직관적입니다.
...
## 알려진 한계: `excited`가 기본값처럼 자주 나옴

실제로 써보면 셋 중 **`excited`(신남)가 가장 자주 뜨는 경향**이 있습니다. 이 모델은 "아무 자세도 아님(중립)" 클래스가 따로 없어서, 애매한 자세도 반드시 thinking/excited/tired 중 하나로 강제 분류해야 합니다. 그런데 `excited`를 가르는 기준(`hands_raised`)은 손이 살짝만 올라가도 넘기기 쉬운 반면, `thinking`/`tired`는 손이 얼굴에 닿을 만큼 가까워야 해서 기준이 더 까다롭습니다. 그 결과 애매한 자세들이 `excited` 쪽으로 쏠리게 됩니다.

개선하려면 "중립(neutral)" 클래스를 하나 추가해서 4종 분류로 만들거나, `hands_raised`의 판정 기준을 더 엄격하게 조정하는 방법이 있습니다.
...
└── pose_img/person/
    ├── classes.json          # ["thinking", "excited", "tired"]
    ├── thinking/ excited/ tired/   # 클래스별 수집 이미지

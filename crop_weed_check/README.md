...
이 파이프라인이 **어떤 조건에서 잘 되고 어떤 조건에서 안 되는지**를 명확히 보여주기 위해, 성격이 다른 사진·영상 12개를 모아 전부 돌려봤습니다.

아래는 이미지 기반 테스트 7건의 실제 `/api/analyze` 응답값(방제 대상 수 / 전체 탐지 개체 수)을 그대로 뽑아 비교한 그래프입니다. "방제 대상 비율"이 낮을수록 판정이 안정적이라는 뜻인데, 성공 사례(대두밭 0%, 옥수수밭 사진 7.9%)와 실패 사례(16~28%)가 뚜렷하게 갈리는 걸 볼 수 있습니다:

![테스트 결과 요약](docs/images/test_summary_chart.png)
...
| 3 | 옥수수밭 사진 (고랑 사이 돌길, 하늘 많이 보임) | ✅ 성공 | 줄이 뚜렷함. 처음엔 하늘 오탐 버그가 있었는데 수정 후 정상 동작 확인 (아래 참고) |
| 4 | 잔디밭 영상 | ❌ 실패 | 밭이 아니라 화면 전체가 균일한 잔디라 구분 대상 자체가 없음 |
| 5 | 지면 눈높이 사진 (아주 낮은 각도) | ❌ 실패 | 고랑 구조가 안 보이고 원근 때문에 판정이 사실상 무작위 |
| 6 | 고고도 드론 곡물밭 영상 | ❌ 실패 | 너무 높아서 화면 전체가 "하나의 큰 초록 덩어리"로만 보임 |
| 7 | 추수 끝난 벼 그루터기밭 영상 | ❌ 실패 | 초록 식생이 거의 없어서(다 갈색) ExG가 찾을 게 없음 |
| 8 | 시든 해바라기밭 영상 (캐노피 높이 촬영) | ❌ 실패 | 위에서 보는 게 아니라 캐노피 옆면을 봐서 고랑이 안 보임 |
| 9 | 촘촘한 양배추밭 사진 | ❌ 실패 | 잎맥·잎 색 굴곡이 심해서 한 포기가 여러 조각으로 과다 분할됨 |
| 10 | 텃밭 모종 사진 (배경에 큰 나무) | ❌ 실패 (새로운 유형) | 전경 작물보다 배경 나무·산울타리가 화면을 더 많이 차지해서 그쪽이 "고랑"으로 오인됨 |
| 11 | 간격 넓게 심은 상추·케일밭 | ❌ 실패 (새로운 유형) | "촘촘한 줄=작물, 고립=잡초"라는 밀도 규칙의 전제 자체가 어긋남 — 간격을 두고 심는 채소에는 안 맞는 가정 |
| 12 | 옥수수밭 사진 (지면에서 위로 올려다본 역광 사진) | ❌ 실패 | 역광 때문에 잎 하나하나가 하늘색 틈으로 쪼개져서 784개까지 과다 분할됨 (아래 참고) |

> 참고: 잎 색이 초록이 아닌 작물(예: 적상추처럼 보라·적색 계열)도 테스트해봤는데, ExG가 애초에 "초록색"만 식생으로 인식하기 때문에 그런 작물 자체를 통째로 놓치는 걸 확인했습니다. 이건 판정이 틀리는 수준이 아니라 파이프라인이 다루는 범위 밖이라 판단해 표에서는 제외했습니다 — ExG 대신 색상 불문 식생 지수를 쓰거나 학습 모델로 교체해야 해결되는 문제입니다.

## 겪었던 문제: 파란 하늘·구름도 식생으로 오인 (버그, 수정함)

옥수수밭 사진(고랑 사이 돌길, 하늘이 화면 절반 가까이 보임)을 테스트하다가, **파란 하늘·구름 경계에도 노란/빨간 박스가 잔뜩 찍히는** 문제를 발견했습니다. 확인해보니 구름 경계의 JPEG 압축 아티팩트 픽셀 일부가 `R≈0, G≈130, B≈220`처럼 R이 거의 0으로 깨져 있어서, `ExG = 2G - R - B` 계산에서 우연히 양수가 나와 식생으로 오탐된 것이었습니다.
...
역광 사진(위 12번)은 옥수수 대가 20~30개 정도밖에 안 보이는데 784개로 과다 분할됐습니다. 파라미터 튜닝으로 고칠 수 있는지 직접 실험해봤습니다 — `segmentation.py`의 식생 마스크에 모폴로지 닫힘(closing) 커널을 더 크게 적용해서 잎 사이 하늘색 틈을 메워봤습니다.
...
커널을 키울수록 줄어들긴 하지만, 25×25까지 키워도 여전히 실제 포기 수(20~30개)의 7배가 넘습니다. 더 키우면 이 사진은 나아지겠지만, **다른 정상적인 사진에서는 서로 떨어진 포기들을 하나로 잘못 합쳐버리는 부작용**이 생깁니다(파라미터 하나를 전체 파이프라인이 공유하기 때문). 즉 이 사진만을 위해 설정을 바꾸면 다른 성공 사례들이 망가지는 트레이드오프가 있어서, 설정값 튜닝으로는 해결이 안 된다는 결론을 내렸습니다 — 역광 자체를 보정하는 전처리(예: 노출 보정)나 학습 기반 세그멘테이션으로 가야 하는 문제입니다.
...
4. **작물이 촘촘한 줄로 심겨있어야 함** (밀도 기반 판정 규칙의 전제 — 간격 넓게 심는 채소는 안 맞음)
5. **화면을 지배하는 무관한 배경 식생이 없어야 함** (배경 나무·산울타리가 전경보다 크면 그쪽이 고랑으로 오인됨)
...
**수정**: `detection.py`에서 이미지 전체 면적의 8%보다 큰 덩어리는 "개별 포기로 볼 수 없다"고 보고 애초에 후보에서 제외하도록 했습니다. 이후 다시 테스트하니 비정상적으로 큰 박스는 사라지고, 정상 크기의 박스들 중 하나가 올바르게 방제 대상으로 표시됐습니다 (이후 아래에서 crop/weed 판정 기준 자체를 또 한 번 바꾸면서 결과가 더 달라집니다).
...
python pipeline/benchmark.py --video sample_data/FarmField.mp4
```

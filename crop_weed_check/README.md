...
이 파이프라인이 **어떤 조건에서 잘 되고 어떤 조건에서 안 되는지**를 명확히 보여주기 위해, 성격이 다른 사진·영상 11개를 모아 전부 돌려봤습니다.

아래는 이미지 기반 테스트 6건의 실제 `/api/analyze` 응답값(방제 대상 수 / 전체 탐지 개체 수)을 그대로 뽑아 비교한 그래프입니다. "방제 대상 비율"이 낮을수록 판정이 안정적이라는 뜻인데, 성공 사례(대두밭 0%, 옥수수밭 사진 7.9%)와 실패 사례(16~28%)가 뚜렷하게 갈리는 걸 볼 수 있습니다:
